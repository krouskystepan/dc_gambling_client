...
const MultipleSelector = ({
  ...
  useEffect(() => {
    ...
    const filteredOptions = onSearchSync(debouncedSearchTerm || '')
    setOptions(transToGroupOption(filteredOptions, groupBy))
  }, [debouncedSearchTerm, groupBy, open, onSearchSync])
  ...
  // calculate selectable options first
  ...
  const EmptyItem = React.useCallback(() => {
    ...
    // flatten options
    const allOptions = Object.values(selectables).flat()
    if (allOptions.length === 0) {
      return <CommandEmpty>{emptyIndicator}</CommandEmpty>
    }
    return null
  }, [emptyIndicator, selectables])
